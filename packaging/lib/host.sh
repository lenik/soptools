...
#
# Environment:
#   SOPTOOLS_SRCDIR       project root (default: packaging/../..)
#   SOPTOOLS_FORCE_LOCAL  skip can-local check (set on a build host by gh-makerelease)
set -euo pipefail

_LIBDIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
SOPTOOLS_SRCDIR=${SOPTOOLS_SRCDIR:-$(cd "$_LIBDIR/../.." && pwd)}
...
find_build_host_file() {
    ...
    for k in $(_kind_fallbacks "$kind"); do
        for base in "$SOPTOOLS_SRCDIR/.config/soptools" "$HOME/.config/soptools"; do
...
cmd_dump() {
    local file=${1:-}
    if [[ -z $file ]]; then
        file=$(find_build_host_file "${SOPTOOLS_PACKAGING:-win32}") || {
    ...
    if ((BH_N > 0)); then
        ...
        echo "last_shell=$(last_shell "${SOPTOOLS_PACKAGING:-}")"
...
cmd_run() {
    ...
    if [[ ${SOPTOOLS_FORCE_LOCAL:-} == 1 ]]; then
        exec "$@"
    fi
    ...
    local k
    for k in $(_kind_fallbacks "$kind"); do
        echo "    $SOPTOOLS_SRCDIR/.config/soptools/$k.build-host" >&2
        echo "    $HOME/.config/soptools/$k.build-host" >&2
    done
    return 2
}
