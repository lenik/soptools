...
bool SopEngine::run_shell(const std::string &step_id) {
    ...
    std::thread([this, step_id, run, script = step.shell_script]() {
        run->running = true;
        run->log += "$ cd " + opts_.project_dir + "\n";
        run->log += script;
        ...
        const std::string cmd =
            "cd " + opts_.project_dir + " && bash -lc " + std::string(1, '"') + script + '"';
        FILE *pipe = popen(cmd.c_str(), "r");
        if (!pipe) {
            run->log += "failed to start shell\n";
            run->exit_code = 127;
            run->running = false;
            ensure_sop_config_dirs(opts_.project_dir, opts_.sop_dir);
            const std::string log_path = sop_step_log_path(opts_.project_dir, opts_.sop_dir, step_id);
